...
tmux send-keys -t 0 "roslaunch kybot_bringup kybot_minimal.launch --screen" Enter
...
#tmux send-keys -t 1 "roslaunch kybot_bringup odom_ekf.launch" Enter

## 直接执行kybot文件夹下的该脚本应该不带参数，参数个数为0；执行pc文件夹下的同名脚本来执行kybot下的该脚本，需要带参数执行，参数随意，默认为1
#if [ $# = 0 ];then
        #sleep 1
        ...
        #tmux send-keys -t :2 "rosrun rviz rviz -d `rospack find kybot_nav`/rviz/nav_ekf.rviz" Enter
#fi
if [ $# = 0 ];then
        sleep 1
        ...
        tmux select-window -t :1
        tmux send-keys -t :1 "source $this_script_dir/env.sh" Enter
        tmux send-keys -t :1 "rosrun rviz rviz -d `rospack find kybot_nav`/rviz/nav_odom.rviz" Enter
fi
